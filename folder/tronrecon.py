...
def run_shodan_scan(ip):
    try:
        status(f"[Shodan] Querying Shodan for {ip}...")
...
def main():
    ...
    for target in alive_subs:
        ...
        status(f"[Shodan] Fetching data for {ip}...")
        shodan_results[ip] = run_shodan_scan(ip)

        status(f"Scanning ports for {domain} [{ip}]...")
        open_ports = scan_ports(ip, ports)
        port_results[domain] = open_ports

    nuclei_results = run_nuclei_scan(alive_subs + [f"http://{socket.gethostbyname(t.split('//')[-1].split('/')[0])}:{p}" for t in alive_subs for p in port_results.get(t.split("//")[-1].split("/")[0], [])])
    ...
    for target in alive_subs:
        ...
        domain = target.split("//")[-1].split("/")[0]
        for port in port_results.get(domain, []):
            url = f"http://{socket.gethostbyname(domain)}:{port}"
